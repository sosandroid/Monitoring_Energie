...
flowchart LR;
    ...
    I(Open-dpe.fr fa:fa-file-code)-->C;
...
- Ajout des prévision tempo avec Open-dpe.fr
...
Créez la configuration `config.json` pour le paramétrage en vous basant sur `config - sample.json`. Mettez ce fichier dans le dossier `json`. Il vous faut une appId pour OWM, la configuration de votre SMTP et le paramétrage de la liste des lieux / destinataires.  
Rendez les fichiers Python exécutables `chmod -R a+x *.py`
