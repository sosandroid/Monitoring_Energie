<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1" /></head><body style="font-family: Helvetica, sans-serif; font-size:0.8em; min-width:300px;margin:auto;padding:15px;background: #e6f7ff;">
<div id="content">
<p>Bonjour les Suzois,</p>
<p>Il est __heure__, le soleil se lève à __sunrise__ et se couche à __sunset__.<br>Les dernières infos de couleur du linky:</p>
<div style="width:100%; height:auto; text-align: center; margin-left: auto; margin-right: auto;"><div style="min-width: 70px; width:auto; max-width:150px;height:auto; margin: auto; border: 1px solid; border-radius: 10px; padding: 3px;font-weight:bold;margin-bottom:2px;__stylej__">aujourd'hui : __couleurj__</div><div style="min-width: 70px; width:auto; max-width:150px;height:auto; margin: auto; border: 1px solid; border-radius: 10px; padding: 3px;font-weight:bold;margin-bottom:2px;__stylej1__">demain : __couleurj1__</div></div>
<div class="clear" style="clear: both;"></div>
<p>Il restera à planifier <b>__nbrouges__</b> jours rouges et <b>__nbblancs__</b> jours blancs</p>
<p>La météo des 24 prochaines heures</p>
<div class="meteos" style="width:auto;margin: auto;">
__meteohtml__
</div></div></body></html>